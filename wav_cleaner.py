...
def iter_chunks(buf):
    ...
    while pos + 8 <= end:
        ...
        if data_end > end:
            raise ValueError(
                f"Повреждённый WAV: чанк {cid!r} на смещении {pos} "
                f"заявляет размер {size} байт, но в файле столько нет"
            )
...
def should_remove_chunk(chunk):
    ...
    return chunk.id in REMOVE_CHUNKS
...
def clean_wav(path):
    ...
    if riff_type == RF64:
        raise ValueError(
            "RF64 WAV (файлы >4 ГБ) пока не поддерживаются: "
            "безопасная пересборка ds64-чанка не реализована"
        )
